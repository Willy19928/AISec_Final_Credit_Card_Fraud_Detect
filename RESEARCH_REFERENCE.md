...
| Automated security checks in CI/CD | Repository CI verifies the artifact manifest. The server CI regenerates its deployment manifest and runs API and artifact-validation tests. | `.github/workflows/artifact-integrity.yml`, deployment-server `server-ci.yml` |
